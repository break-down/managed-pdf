...
using BreakDown.ManagedPdf.Html.Core.Handlers;
using BreakDown.ManagedPdf.Html.Core.Parse;
...
namespace BreakDown.ManagedPdf.Html.Core.Dom
{
    ...
    internal class CssBox : CssBoxProperties, IDisposable
    {
        ...
        public void ParseToWords()
        {
            ...
            while (startIdx < _text.Length)
            {
                ...
                if (startIdx < _text.Length)
                {
                    ...
                    {
                        endIdx = startIdx;
                        while (endIdx < _text.Length && !char.IsWhiteSpace(_text[endIdx]) && _text[endIdx] != '-' && WordBreak != CssConstants.BreakAll &&
                        ...
                        if (endIdx < _text.Length &&
                            (_text[endIdx] == '-' || WordBreak == CssConstants.BreakAll || CommonUtils.IsAsianCharecter(_text[endIdx])))
                        {
                            endIdx++;
                        }
                        ...
                        if (endIdx > startIdx)
                        {
                            ...
                            _boxWords.Add(new CssRectWord(this, HtmlUtils.DecodeHtml(_text.Substring(startIdx, endIdx - startIdx)), hasSpaceBefore,
                                                          hasSpaceAfter));
                        }
                    }
        ...
        protected virtual void PerformLayoutImp(RGraphics g)
        {
            ...
            if (IsBlock || Display == CssConstants.ListItem || Display == CssConstants.Table || Display == CssConstants.InlineTable ||
                Display == CssConstants.TableCell)
            {
                ...
                if (Display != CssConstants.TableCell)
                {
                    ...
                    else
                    {
                        ...
                        top = (prevSibling == null && ParentBox != null ? ParentBox.ClientTop : ParentBox == null ? Location.Y : 0) +
                              MarginTopCollapse(prevSibling) + (prevSibling != null ? prevSibling.ActualBottom + prevSibling.ActualBorderBottomWidth : 0);
        ...
        protected double MarginTopCollapse(CssBoxProperties prevSibling)
        {
            ...
            else if (_parentBox != null && ActualPaddingTop < 0.1 && ActualPaddingBottom < 0.1 && _parentBox.ActualPaddingTop < 0.1 &&

...
namespace BreakDown.ManagedPdf.Html.Core.Utils
{
    ...
    internal sealed class DomUtils
    {
        ...
        public static CssBox GetPreviousSibling(CssBox b)
        {
            if (b.ParentBox != null)
            {
                ...
                if (index > 0)
                {
                    ...
                    while ((sib.Display == CssConstants.None || sib.Position == CssConstants.Absolute || sib.Position == CssConstants.Fixed) &&
                           index - diff - 1 >= 0)
                    {
        ...
        public static CssBox GetPreviousContainingBlockSibling(CssBox b)
        {
            ...
            while (conBlock.ParentBox != null && index < 1 && conBlock.Display != CssConstants.Block && conBlock.Display != CssConstants.Table &&
                   conBlock.Display != CssConstants.TableCell && conBlock.Display != CssConstants.ListItem)
            {
        ...
        private static void WriteHtml(CssParser cssParser,
                                      StringBuilder sb,
                                      CssBox box,
